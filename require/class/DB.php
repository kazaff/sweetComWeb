...
class DB{
	...
	public function __construct() {
		...
		require_once(Root_Path."/configs/dbInfo.php");
		$this->connect($_db['host'].':'.$_db['port'], $_db['account'], $_db['password'], $_db['dbName'], $_db['pconnect']);
		$this->is_log = $_db['log'];
		if($this->is_log){
			$handle = fopen(Root_Path."/logs/dblog.txt", "a+");
			$this->handle=$handle;
		}
	}
