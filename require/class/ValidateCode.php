...
/**
 * 使用验证码类的方法： 
 * $vc = new Authnum(验证码长度,图片宽度,图片高度); 
 * 实例化时不带参数则默认是四位的60*25尺寸的常规验证码图片 
 * 表单页面检测验证码的方法，对比 $_SESSION[verifyCode] 是否等于 $_POST[验证码文本框ID] 
 * 可选配置： 
 * 1.验证码类型：$vc->ext_num_type=1; 值为1是小写类型，2是大写类型，3是数字类型 
 * 2.干扰点：$vc->ext_pixel = false; 值为false表示不添加干扰点 
 * 3.干扰线：$vc->ext_line = false; 值为false表示不添加干扰线 
 * 4.Y轴随机：$vc->ext_rand_y = false; 值为false表示不支持图片Y轴随机 
 * 5.图片背景：改变 $red $green $blue 三个成员变量的值即可 

	$vc = new Authnum(); 
	$vc->ext_num_type=''; 
	$vc->ext_pixel = true; //干扰点 
	$vc->ext_line = false; //干扰线 
...
class Authnum{ 
	...
	**/ 
	...
	function get_randnum () { 
		...
		$_SESSION['verifyCode'] = $this->randnum; 
	} 
